...
class OpsGenieNotificationView(BaseModel):
    ...
    ops_genie_api_key: Optional[StrictStr] = Field(None, alias="opsGenieApiKey", description="API Key that MongoDB Cloud needs to send this notification via Opsgenie. The resource requires this parameter when `\"notifications.[n].typeName\" : \"OPS_GENIE\"`. If the key later becomes invalid, MongoDB Cloud sends an email to the project owners. If the key remains invalid, MongoDB Cloud removes it.  **NOTE**: After you create a notification which requires an API or integration key, the key appears partially redacted when you:  * View or edit the alert through the Atlas UI.  * Query the alert for the notification through the Atlas Administration API.")
    ops_genie_region: Optional[StrictStr] = Field('US', alias="opsGenieRegion", description="Opsgenie region that indicates which API Uniform Resource Locator (URL) to use.")
    type_name: StrictStr = Field(..., alias="typeName", description="Human-readable label that displays the alert notification type.")
    __properties = ["delayMin", "intervalMin", "opsGenieApiKey", "opsGenieRegion", "typeName"]

    @validator('ops_genie_region')
    def ops_genie_region_validate_enum(cls, v):
        if v is None:
            return v
    ...
    @classmethod
    def from_json(cls, json_str: str) -> OpsGenieNotificationView:
        """Create an instance of OpsGenieNotificationView from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> OpsGenieNotificationView:
        """Create an instance of OpsGenieNotificationView from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return OpsGenieNotificationView.parse_obj(obj)

        _obj = OpsGenieNotificationView.parse_obj({
            "delay_min": obj.get("delayMin"),
            "interval_min": obj.get("intervalMin"),
            "ops_genie_api_key": obj.get("opsGenieApiKey"),
            "ops_genie_region": obj.get("opsGenieRegion") if obj.get("opsGenieRegion") is not None else 'US',
