...
class CustomDBRole(BaseModel):
    ...
    role_name: constr(strict=True) = Field(..., alias="roleName", description="Human-readable label that identifies the role for the request. This name must be unique for this custom role in this project.")
    __properties = ["actions", "inheritedRoles", "roleName"]

    @validator('role_name')
    def role_name_validate_regular_expression(cls, v):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CustomDBRole:
        """Create an instance of CustomDBRole from a JSON string"""
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CustomDBRole:
        """Create an instance of CustomDBRole from a dict"""
        if obj is None:
            return None

        if type(obj) is not dict:
            return CustomDBRole.parse_obj(obj)

        _obj = CustomDBRole.parse_obj({
