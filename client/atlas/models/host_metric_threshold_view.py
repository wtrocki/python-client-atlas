...
HOSTMETRICTHRESHOLDVIEW_ONE_OF_SCHEMAS = ["DataMetricThresholdView", "NumberMetricThresholdView", "RawMetricThresholdView", "TimeMetricThresholdView"]

class HostMetricThresholdView(BaseModel):
    ...
    # data type: NumberMetricThresholdView
    oneof_schema_4_validator: Optional[NumberMetricThresholdView] = None
    actual_instance: Any
    one_of_schemas: List[str] = Field(HOSTMETRICTHRESHOLDVIEW_ONE_OF_SCHEMAS, const=True)
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        # validate data type: NumberMetricThresholdView
        if type(v) is not NumberMetricThresholdView:
            error_messages.append(f"Error! Input type `{type(v)}` is not `NumberMetricThresholdView`")
        else:
            match += 1

        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into HostMetricThresholdView with oneOf schemas: DataMetricThresholdView, NumberMetricThresholdView, RawMetricThresholdView, TimeMetricThresholdView. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into HostMetricThresholdView with oneOf schemas: DataMetricThresholdView, NumberMetricThresholdView, RawMetricThresholdView, TimeMetricThresholdView. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> HostMetricThresholdView:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> HostMetricThresholdView:
        ...
        # check if data type is `NumberMetricThresholdView`
        if _data_type == "NumberMetricThresholdView":
            instance.actual_instance = NumberMetricThresholdView.from_json(json_str)
            return instance
        ...
        try:
            ...
            error_messages.append(str(e))
        # deserialize data into NumberMetricThresholdView
        try:
            instance.actual_instance = NumberMetricThresholdView.from_json(json_str)
            match += 1
        except ValidationError as e:
